##########################################################################################
#R Code for Chapter 14 of: Field,A. P.,Miles,J. N. V.,& Field,Z. C. (2012). Discovering Statistics Using R: and Sex and Drugs and Rock 'N' Roll. #London Sage (c) 2011 Andy P. Field,Jeremy N. V. Miles & Zoe C. Field
##########################################################################################
...
#Exploring Data
dateBoxplot<-ggplot(speedData,aes(looks,dateRating,colour=personality))+geom_boxplot()+labs(x="Attractiveness",y="Mean Rating of Date",colour="Charisma")+facet_wrap(~gender)
looksBar<-ggplot(speedData,aes(looks,dateRating))+stat_summary(fun.y=mean,geom="bar",fill="White",colour="Black")+stat_summary(fun.data=mean_cl_boot,geom="pointrange")+labs(x="Attractiveness",y="Mean Rating of Date") 
charismaBar<-ggplot(speedData,aes(personality,dateRating))+stat_summary(fun.y=mean,geom="bar",fill="White",colour="Black")+stat_summary(fun.data=mean_cl_boot,geom="pointrange")+labs(x="Charisma",y="Mean Rating of Date") 
genderBar<-ggplot(speedData,aes(gender,dateRating))+stat_summary(fun.y=mean,geom="bar",fill="White",colour="Black")+stat_summary(fun.data=mean_cl_boot,geom="pointrange")+labs(x="Gender",y="Mean Rating of Date") 
genderLooks<-ggplot(speedData,aes(looks,dateRating,colour=gender))+stat_summary(fun.y=mean,geom="point")+stat_summary(fun.y=mean,geom="line",aes(group= gender))+stat_summary(fun.data=mean_cl_boot,geom="errorbar",width=0.2)+labs(x="Attractiveness",y="Mean Rating of Date",colour="Gender")+scale_y_continuous(limits=c(0,100)) 
genderCharisma<-ggplot(speedData,aes(personality,dateRating,colour=gender))+stat_summary(fun.y=mean,geom="point")+stat_summary(fun.y=mean,geom="line",aes(group= gender))+stat_summary(fun.data=mean_cl_boot,geom="errorbar",width=0.2)+labs(x="Charisma",y="Mean Rating of Date",colour="Gender")+scale_y_continuous(limits=c(0,100)) 
looksCharisma<-ggplot(speedData,aes(looks,dateRating,colour=personality))+stat_summary(fun.y=mean,geom="point")+stat_summary(fun.y=mean,geom="line",aes(group= personality))+stat_summary(fun.data=mean_cl_boot,geom="errorbar",width=0.2)+labs(x="Attractiveness",y="Mean Rating of Date",colour="Charisma")+scale_y_continuous(limits=c(0,100)) 
looksCharismaGender<-ggplot(speedData,aes(looks,dateRating,colour=personality))+stat_summary(fun.y=mean,geom="point")+stat_summary(fun.y=mean,geom="line",aes(group= personality))+stat_summary(fun.data=mean_cl_boot,geom="errorbar",width=0.2)+labs(x="Attractiveness",y="Mean Rating of Date",colour="Charisma")+scale_y_continuous(limits=c(0,100))+facet_wrap(~gender)
#using ezAnova
...
pictureData<-field_examples$ProfilePicture.dat
names(pictureData)<-c("case","relationship_status","With Man","Alone")
pictureData$row<-c(1:17,1:23)
profileMelt<-reshape::melt(pictureData,id=c("case","row","relationship_status"),measured=c("couple","alone"))
...
profileGraph<-ggplot(profileMelt,aes(relationship_status,friend_requests,colour=profile_picture))+stat_summary(fun.y=mean,geom="point")+stat_summary(fun.y=mean,geom="line",aes(group= profile_picture))+stat_summary(fun.data=mean_cl_boot,geom="errorbar",width=0.2)+labs(x="Relationship Status",y="Number of Friend Requests",colour="Contents of Profile Picture")+scale_y_continuous(limits=c(0,10)) 
...
#-------labcoat leni-----
#Read in the data:
jealousData<-field_examples$`Schutzwohl(2008).dat`
...
jealousLong<-reshape::melt(jealousData,id=c("Participant","Relationship","Age","Distractor_Colour","Gender"),measured=c("Distracter_Neutral","Distracter_Emotional","Distracter_Sexual","Target_Neutral","Target_Emotional","Target_Sexual"))
...
#Exploring Data
#Male Boxplot:
maleBoxplot<-ggplot(malesOnly,aes(Distracter_Type,Sentences_Remembered,colour=Sentence_Type))+geom_boxplot()+labs(x="Distracter Type",y="Mean Number of Sentences Remembered",colour="Sentence Type")+facet_wrap(~Relationship)
#Female Boxplot:
femaleBoxplot<-ggplot(femalesOnly,aes(Distracter_Type,Sentences_Remembered,colour=Sentence_Type))+geom_boxplot()+labs(x="Distracter Type",y="Mean Number of Sentences Remembered",colour="Sentence Type")+facet_wrap(~Relationship)
#Descriptives
...
#-------Smart Alex Task 1-----
#Read in the data:
mixedAttitude<-field_examples$MixedAttitude.dat
...
#Exploring Data
#Boxplot:
attitudeBoxplot<-ggplot(attitudeLong,aes(Drink,Drink_Rating,colour=Imagery))+geom_boxplot()+labs(x="Drink",y="Mean Drink Rating",colour="Imagery")+facet_wrap(~Gender)
#Descriptives
...
#-------Smart Alex Task 2-----
#Read in the data:
textMessages<-field_examples$TextMessages.dat
...
#-------Smart Alex Task 3-----
#Read in the data:
bigBrother<-field_examples$BigBrother.dat
