...
album1<-field_examples$`Album Sales 1.dat`
#----run the simple linear regression model---
albumSales.1<-lm(sales ~ adverts,data=album1)
summary(albumSales.1)
sqrt(0.3346)
#----access the album2 data----
album2<-field_examples$`Album Sales 2.dat`
...
histogram<-ggplot(album2,aes(studentized.residuals))+geom_histogram(aes(y=..density..),colour="black",fill="white")+labs(x="Studentized Residual",y="Density")+stat_function(fun=dnorm,args=list(mean=mean(album2$studentized.residuals,na.rm=TRUE),sd=sd(album2$studentized.residuals,na.rm=TRUE)),colour="red",size=1)
scatter<-ggplot(album2,aes(fitted,studentized.residuals))+geom_point()+geom_smooth(method="lm",colour="Red")+labs(x="Fitted Values",y="Studentized Residual") 
...
gfr<-field_examples$GlastonburyFestivalRegression.dat
...
PersonalityData<-field_examples$`Chamorro-Premuzic.dat`
...
Supermodel<-field_examples$Supermodel.dat
...
ChildAggression<-field_examples$ChildAggression.dat
