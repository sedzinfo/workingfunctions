...
setwd("/home/dimitrios/Dropbox (Psycholate)/dimitrios/working/R/scripts/field/data")
imageDirectory<-"/home/dimitrios/Dropbox (Psycholate)/dimitrios/working/R/scripts/field/data"
######Install packages
...
hist.logneg<-ggplot(fieldData,aes(logneg))+opts(legend.position="none")+geom_histogram(aes(y=..density..),colour="black",fill="white")+labs(x="Log transformation of negative Info",y="Density")+stat_function(fun=dnorm,args=list (mean=mean (fieldData$logneg,na.rm=TRUE),sd=sd(fieldData$logneg,na.rm=TRUE)),colour="black",size=1)
hist.logneg
hist.logpos<-ggplot(fieldData,aes(logpos))+opts(legend.position="none")+geom_histogram(aes(y=..density..),colour="black",fill="white")+labs(x="Log transformation of Positive Info",y="Density")+stat_function(fun=dnorm,args=list (mean=mean (fieldData$logpos,na.rm=TRUE),sd=sd(fieldData$logpos,na.rm=TRUE)),colour="black",size=1)
hist.logpos
hist.lognone<-ggplot(fieldData,aes(lognone))+opts(legend.position="none")+geom_histogram(aes(y=..density..),colour="black",fill="white")+labs(x="Log transformation of No Info",y="Density")+stat_function(fun=dnorm,args=list (mean=mean (fieldData$lognone,na.rm=TRUE),sd=sd(fieldData$lognone,na.rm=TRUE)),colour="black",size=1)
hist.lognone
#Robust Anova:
