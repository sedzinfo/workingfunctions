...
# 0. Setup
# For a given color,define a text color that will have good contrast. Examples:
...
# Define this array of text contrast colors that correponds to each member of the colors() array.
TextContrastColor<-unlist(lapply(colors(),SetTextContrastColor))
# 1a. Plot matrix of R colors,in index order,25 per row. This example plots each row of rectangles one at a time.
...
# 1b. Plot matrix of R colors,in "hue" order,25 per row. This example plots each rectangle one at a time.
...
# Define string vector of RGB hex and decimal constants for given color as a string. Example:
