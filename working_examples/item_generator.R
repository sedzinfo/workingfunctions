...
sequences <- rbind(
  data.frame(matrix(unlist(add), nrow = length(add), byrow = T)),
...
item_generator <- function(stem_text = stem_text, formulae = formulae, N = N, C, options_affix, ans_key, save.csv) {
  ...
  if (length(inputs_frame) == length(raw_vrbles)) {
    ...
    stop("Declared variables in stem should be kept in the format of [.] without space inside and declared variables in stem must be similar to the input variables.")
  }
